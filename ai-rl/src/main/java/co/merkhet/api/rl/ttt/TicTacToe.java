 ...
 * Making the GUI and creating other edits for the game--D-Studios(Devang
 * Bhatnagar)
 *
 *
 */
...
public class TicTacToe extends JPanel {
	static String[] board;
	static String turn;
	// A list of buttons is created and, when clicked, can change array values in
	// board[]. This will be used to see if someone won or if a draw occurred.
	static List<JButton> buttons = new ArrayList<JButton>();
	static JFrame frame = new JFrame();
	static int value = 0;
	static String winner = null;
	static JLabel information = new JLabel(
			"<html>Welcome to 2 Player Tic Tac Toe.<br/>X will play first.<br/> Click near the center of a slot to place X in:");

	static TicTacToeAgent agent;

	// In the main function, a JFrame window is being created and the lines from the
	// paintComponent function are drawn. Then, populateEmptyBoard is called.
	...
	public static void externalRun(TicTacToeAgent p1) {
		agent = p1;
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		TicTacToe ticTacToe = new TicTacToe();
		frame.add(ticTacToe);
		...
		populateEmptyBoard();
		if (turn.equals("O")) {
			Pair<Integer, Integer> action = agent.playAction(getEnvironment());
			int i = action.getValue0();
			int j = action.getValue1();
			int indx = i * 3 + j;
			board[indx] = "O";
			buttons.get(indx).setText("O");
			turn = "X";
		}
	}

	// In winningText, checkWinner is called to see if a player has won. If the
	// player won or a draw occurred, the label on the window is updated to show it.
...
//In this function, the board array is used to see if there is a winner or if a draw occurred. If both of these conditions are false, the game continues.
	static String checkWinner() {
		...
		information
				.setText("<html>" + turn + "'s turn.<br/> Click near the center of a slot to place " + turn + " in.");
		return null;
	}
...
//In this function, buttons are created for the player to put his/her/their turns in. Every time a player clicks on an applicable button, the board array is updated.
					...
					public void actionPerformed(ActionEvent e) {
						if (winner == null && board[val] == "Empty") {
							...
							if (turn.equals("X")) {
								...
								int indx = i * 3 + j;
								board[indx] = "X";
								buttons.get(indx).setText("X");
							}
							winningText();
						} else if (winner != null)
							information.setText("The game has already been finished!");
						else
							information.setText("<html>The slot is already taken!<br/> Pick another slot!");
					}
	...
	private static Environment getEnvironment() {
		...
		for (int i = 0; i < board.length; i++) {
			if (board[i] == "Empty") {
				environment.setBoard(j, k, 0);
			} else if (board[i] == "X") {
				environment.setBoard(j, k, -1);
			} else if (board[i] == "O") {
				environment.setBoard(j, k, 1);
			}
			if (i == 2) {
