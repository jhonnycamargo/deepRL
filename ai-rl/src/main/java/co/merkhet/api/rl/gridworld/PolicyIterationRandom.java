...
public class PolicyIterationRandom {
	...
	public static void main(String[] args) {
		Grid grid = Grid.negativeGrid(-1.0f);
			...
			while (true) {
				...
				for (Pair<Integer, Integer> s : states) {
					...
					if (policy.getInt(s.getValue0(), s.getValue1()) != 0) {
						for (GridAction a : GridAction.values()) {
							double p;
							if (a.getAction() == policy.getInt(s.getValue0(), s.getValue1())) {
								p = 0.5;
							} else {
								p = 0.5 / 3;
							}
							grid.setState(s);
							float r = grid.move(a);
							newV += p * (r + GAMMA * upperV.getDouble(grid.currentState().getValue0(),
									grid.currentState().getValue1()));
						}
			...
			for (Pair<Integer, Integer> s : states) {
				if (policy.getInt(s.getValue0(), s.getValue1()) != 0) {
					...
					for (GridAction action : GridAction.values()) {
						double v = 0;
						for (GridAction action2 : GridAction.values()) {
							double p;
							if (action == action2) {
								p = 0.5;
							} else {
							...
							grid.setState(s);
							float r = grid.move(action2);
							v += p * (r + GAMMA * upperV.getDouble(grid.currentState().getValue0(),
									grid.currentState().getValue1()));
						}
						if (v > bestValue) {
							bestValue = v;
